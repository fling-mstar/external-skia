...
static bool Sample_RGBA_D8888_withoutPreMultiplyAlpha(void* SK_RESTRICT dstRow,
                              const uint8_t* SK_RESTRICT src,
                              int width, int deltaSrc, int, const SkPMColor[]) {
    SkPMColor* SK_RESTRICT dst = (SkPMColor*)dstRow;
    unsigned alphaMask = 0xFF;
    for (int x = 0; x < width; x++) {
        unsigned alpha = src[3];
        dst[x] = src[0]|(src[01]<<8)|(src[2]<<16)|(src[3]<<24);
        src += deltaSrc;
        alphaMask &= alpha;
    }
    return alphaMask != 0xFF;
...
// MStar Android Patch Begin
static bool Sample_RGBA_D4444_withoutPreMultiplyAlpha(void* SK_RESTRICT dstRow,
                              const uint8_t* SK_RESTRICT src,
                              int width, int deltaSrc, int, const SkPMColor[]) {
    SkPMColor16* SK_RESTRICT dst = (SkPMColor16*)dstRow;
    unsigned alphaMask = 0xFF;
    ...
    for (int x = 0; x < width; x++) {
        unsigned alpha = src[3];
        SkPMColor c = src[0]|(src[1]<<8)|(src[2]<<16)|(src[3]<<24);
        dst[x] = SkPixel32ToPixel4444(c);
        src += deltaSrc;
        alphaMask &= alpha;
    }
...
// MStar Android Patch Begin
static bool Sample_RGBA_D4444_D_withoutPreMultiplyAlpha(void* SK_RESTRICT dstRow,
                                const uint8_t* SK_RESTRICT src,
                            int width, int deltaSrc, int y, const SkPMColor[]) {
    SkPMColor16* SK_RESTRICT dst = (SkPMColor16*)dstRow;
    unsigned alphaMask = 0xFF;
...
// MStar Android Patch Begin
bool SkScaledBitmapSampler::beginWithoutPremultiplyAlpha(SkBitmap* dst, SrcConfig sc, bool dither,
                                  const SkPMColor ctable[]) {
    static const RowProc gProcs[] = {
        // 8888 (no dither distinction)
        Sample_Gray_D8888,  Sample_Gray_D8888,
        Sample_RGBx_D8888,  Sample_RGBx_D8888,
        Sample_RGBA_D8888_withoutPreMultiplyAlpha,  Sample_RGBA_D8888_withoutPreMultiplyAlpha,
        ...
        Sample_RGBA_D4444_withoutPreMultiplyAlpha,  Sample_RGBA_D4444_D_withoutPreMultiplyAlpha,
...
// MStar Android Patch Begin
bool SkScaledBitmapSampler::next_saveto_ypos(const uint8_t* SK_RESTRICT src, int s32ToYPos)
{
    SkASSERT((unsigned)fCurrY < (unsigned)fScaledHeight);
    fCurrY = s32ToYPos;
    char* pDestRow = fDstRow + fDstRowBytes*s32ToYPos;
    bool hadAlpha = fRowProc(pDestRow, src + fX0 * fSrcPixelSize, fScaledWidth,
                             fDX * fSrcPixelSize, fCurrY, fCTable);
    return hadAlpha;
}

bool SkScaledBitmapSampler::next_without_step(const uint8_t* SK_RESTRICT src)
{
    SkASSERT((unsigned)fCurrY < (unsigned)fScaledHeight);

    bool hadAlpha = fRowProc(fDstRow, src, fScaledWidth, fSrcPixelSize, fCurrY, fCTable);
    fDstRow += fDstRowBytes;
    fCurrY += 1;
    return hadAlpha;
}
